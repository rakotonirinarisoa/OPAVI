...
namespace apptab.Extension
{
    public class AFB160
    {
		public AFB CreateTOMPROAFB160(bool devise, string codeJ, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            string texteAFB160 = "";
            OPA_HISTORIQUE historique;
			var Fuser = db.OPA_USERS.Where(x => x.LOGIN == user.LOGIN && x.IDSOCIETE == user.IDSOCIETE).FirstOrDefault();

			/*********             * 0302       * *******/

			var donneurOrde = (from dordre in db.OPA_DONNEURORDRE
                               where dordre.IDSOCIETE == user.IDSOCIETE && dordre.APPLICATION == "COMPTA"
            ...
            try
            {
                var incr = (from incrmentation in db.OPA_BASE
                            where incrmentation.IDSOCIETE == user.IDSOCIETE
            ...
            var beneficiaires = (from dordre in db.OPA_REGLEMENT
                                 where dordre.IDSOCIETE == user.IDSOCIETE && dordre.APPLICATION == "TOMPRO"
            ...
            foreach (var bnfcr in beneficiaires)
            {
                ...
                if (bnfcr.LIBELLE.Length > 11)
                {
					...
					historique.IDSOCIETE = user.IDSOCIETE;
				}
                else
                {
                    texteAFB160 += "0602";
            ...
            try
            {
                OPA_BASE pbase = (from pb in db.OPA_BASE
                                  where pb.IDSOCIETE == user.IDSOCIETE
                                  select pb).First();
            ...
            var nums = (from ecrt in db.OPA_REGLEMENT
                        where ecrt.IDSOCIETE == user.IDSOCIETE && ecrt.APPLICATION == "TOMPRO"
                        select ecrt).ToList();
        ...
        public AFB CreateTOMPAIEAFB160(bool devise, string codeJ, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            string texteAFB160 = "";
            ...
            var donneurOrde = (from dordre in db.OPA_DONNEURORDRE
                               where dordre.IDSOCIETE == user.IDSOCIETE && dordre.APPLICATION == "PAIE"
							   select dordre).First();
            /***********************NOM de fichier************************/
            string nom2 = (from nom in tom.RPROJET
                           select nom.NOM2).First();
            ...
            var nums = (from ecrt in db.OPA_REGLEMENT
                        where ecrt.IDSOCIETE == user.IDSOCIETE && ecrt.APPLICATION == "PAIE"
                        select ecrt).ToList();
            /*foreach (var num in nums)
            {
		...
		public AFB CreateBRAFB160(bool devise, string codeJ, OPA_USERS user)
		{
			
			string s = "";
			OPAVIWEB db = new OPAVIWEB();
            OPA_USERS usr = (from u in db.OPA_USERS
                             where u.LOGIN == user.LOGIN
                             select u).FirstOrDefault();
            OPA_MAPPAGES dbt = (from d in db.OPA_DATABASE
                                     join m in db.OPA_MAPPAGES on d.IDMAPPAGE equals m.ID
                                     where d.IDUSER == usr.ID
                                     select m).FirstOrDefault();

			OPAVITOMATE.connex = "Data Source="+ dbt.INSTANCE+";Initial Catalog="+ dbt.DBASE + ";User ID="+ dbt.CONNEXION+ ";Password="+ dbt.CONNEXPWD+";";
			OPAVITOMATE tom = new OPAVITOMATE();
			...
			var donneurOrde = (from dordre in db.OPA_DONNEURORDRE
							   where dordre.IDSOCIETE == user.IDSOCIETE && dordre.APPLICATION == "BR"
							   select dordre).FirstOrDefault();
			/***********************NOM de fichier************************/
			string nom2 = (from nom in tom.RPROJET
			...
			try
			{
                ...
                else
                {
                    i = 0;
                    y = 0;
                    db.OPA_BASE.Add(new OPA_BASE() { NOMBASE = dbt.DBASE, INCREMENTATION = i, INCRORDREVIR = y, IDSOCIETE = usr.IDSOCIETE});
                    db.SaveChanges();
                }
			}
			catch (Exception ee) { }
			...
			var nums = (from ecrt in db.OPA_REGLEMENTBR
						where ecrt.IDSOCIETE == user.IDSOCIETE && ecrt.APPLICATION == "BR"
						select ecrt).ToList();
			foreach (var num in nums)
			{
        ...
        public  void SaveValideSelectEcriture(List<string> listReg, bool devise, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            //bool test = false;
                ...
                try
                {
                    ...
                    #region Sauve REGELEMENT & ANOMALIE
						...
						if (beneficiaire.COMPTE_BANQUE == null || beneficiaire.BENEFICIAIRE == null)
						{
                            ...
                            panomalie.IDSOCIETE = user.IDSOCIETE;
        ...
        public List<DataListTompro> getREGLEMENT(OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
		...
		public List<DataListTompaie> getREGLEMENTPaie(OPA_USERS user)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			/**************Remplissage dataGridFactSelect*********/

			List<OPA_REGLEMENT> numRegs = db.OPA_REGLEMENT.Where(x=>x.IDSOCIETE == user.IDSOCIETE && x.APPLICATION=="PAIE").ToList();
            //List<OPA_REGLEMENT> numRegs = (from num in db.OPA_REGLEMENT
										  // where num.IDSOCIETE == user.IDSOCIETE
										  // select num).ToList();
		...
		public List<DataListTomOP> getREGLEMENTBR(OPA_USERS user)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			/**************Remplissage dataGridFactSelect*********/
		...
		public List<DataListTompro> getListAnomalie(OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            /**************Remplissage dtAnomalie*******************/
            List<OPA_ANOMALIE> anomalies = (from panom in db.OPA_ANOMALIE
                                            where panom.IDSOCIETE == user.IDSOCIETE
                                            select panom).ToList();
		...
		public List<DataListTomOP> getListAnomalieBR(OPA_USERS user)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			/**************Remplissage dtAnomalie*******************/
        ...
        public bool saveDonneurOrdre(OPA_USERS user, RJL1 djournal, DateTime dateP)// mila modifiena BR ET COMPTA
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            bool test = false;
            ...
            catch (Exception ex)
            {
                ...
                donordre.IDSOCIETE = user.IDSOCIETE;
        ...
        public bool saveDonneurOrdrePaie(OPA_USERS user,string journal,string libDOrdre, string codeGichet, string numCompte, string codeBq)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            bool test = false;
            OPA_DONNEURORDRE donordre = new OPA_DONNEURORDRE();
            var tdonneur1 = (from dord in db.OPA_DONNEURORDRE
                             where dord.IDSOCIETE == user.IDSOCIETE && dord.APPLICATION == "PAIE"                             
                             select dord).FirstOrDefault();
            var projet = (from prjt in tom.RPROJET
                          select new
                          {
		...
		public bool saveDonneurOrdreBR(OPA_USERS user, RJL1 djournal, DateTime dateP)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			bool test = true;
			OPA_DONNEURORDRE donordre = new OPA_DONNEURORDRE();
			var tdonneur1 = (from dord in db.OPA_DONNEURORDRE
							 where dord.IDSOCIETE == user.IDSOCIETE && dord.APPLICATION == "BR"
							 select dord).FirstOrDefault();
			var projet = (from prjt in tom.RPROJET
						  select new
						  {
		...
		public List<DataListTompro> getListEcritureCompta( string journal, DateTime dateD, DateTime dateF, string compteG, string auxi, string auxi1, DateTime dateP, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
      ...
      //                                     }).ToList();
        ...
        public List<DataListTompaie> getListEcriturePaie( string journal, int mois, int annee, string matriculeD, string matriculeF, DateTime dateP, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
            List<DataListTompaie> list = new List<DataListTompaie>();
        ...
        public List<DataListTomOP> getListEcritureBR(string journal, DateTime dateD, DateTime dateF, bool devise, string compteG, string auxi, string etat, DateTime dateP, OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            OPAVITOMATE tom = new OPAVITOMATE();
        ...
        public OPA_DONNEURORDRE getDonneurOrdre(OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            return (from dor in db.OPA_DONNEURORDRE where dor.IDSOCIETE == user.IDSOCIETE select dor).FirstOrDefault();
        }
        private bool removePREGLEMENT(OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            bool test = false;

            List<OPA_REGLEMENT> nums = (from rglm in db.OPA_REGLEMENT
                                        where rglm.IDSOCIETE == user.IDSOCIETE
                                        select rglm).ToList();
        ...
        private bool viderPAnomalie(OPA_USERS user)
        {
            OPAVIWEB db = new OPAVIWEB();
            bool test = false;
            List<OPA_ANOMALIE> anoms = (from anomalie in db.OPA_ANOMALIE
                                        where anomalie.IDSOCIETE == user.IDSOCIETE
		...
		public void SaveValideSelectEcritureBR(List<string> numBR, string journal, string etat, bool devise, OPA_USERS user)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			//bool test = false;
		...
		public void SaveValideSelectEcriturePaie(List<string> listReg, string journal, bool devise, OPA_USERS user)
		{
			OPAVIWEB db = new OPAVIWEB();
			OPAVITOMATE tom = new OPAVITOMATE();
			//bool test = false;
